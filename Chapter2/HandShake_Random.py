...
def handshake_random(ip, port):#定义随机伪装源IP地址三次TCP握手方法，传入目的IP和端口号
	...
	try:#尝试进行连接
		...
		result_raw_synack = sr1(IP(src=source_ip,dst=ip)/TCP(dport=port,sport=source_port,flags=2,seq=init_sn), timeout = 1, verbose = False)
		#响应的数据包产生数组([0]为响应，[1]为未响应)
		tcpfields_synack = result_raw_synack.getlayer(TCP).fields
...
if __name__ == '__main__':
	handshake_random('202.100.1.1', 23)